...
Бот платформы: [tvoytrener](https://max.ru/id110116757261_bot) (`@id110116757261_bot`).
...
1. В MAX откройте бота и нажмите **Запустить**, напишите любое сообщение.
2. Ссылка вида `https://web.max.ru/383353247` — это **`chat_id` диалога**, не `user_id`.
3. `MAX_SUPPORT_CHAT_ID=383353247` (число из ссылки web.max.ru).

Альтернатива: `MAX_SUPPORT_USER_ID` — только если знаете именно numeric `user_id` получателя (не путать с chat_id).
