    ...
    <div className="space-y-6">
      <Card>
        <CardHeader className="space-y-3">
          <div>
            <CardTitle>Город каталога</CardTitle>
            <CardDescription>
              Выберите город — карта создания и история карточек/мероприятий переключаются на него.
            </CardDescription>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="button" size="sm" variant="secondary" onClick={() => scrollToId("admin-catalog-create")}>
                Создать карточку
              </Button>
              <Button type="button" size="sm" variant="outline" onClick={() => scrollToId("admin-catalog-cards")}>
                Карточки каталога
              </Button>
              <Button
                type="button"
                size="sm"
                variant="outline"
                onClick={() => scrollToId("admin-catalog-published")}
              >
                Опубликованные
              </Button>
            </div>
      ...
      <Card id="admin-catalog-create" className="scroll-mt-20">
        <CardHeader>
          <CardTitle>Новая карточка · {selectedCity.name}</CardTitle>
          <CardDescription>
            Одно событие в ленте — несколько инструкторов. Место на карте и фото, затем название.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {unattachedPublished.length ? (
            <div className="space-y-2">
              <p className="text-sm font-medium">Привязать при создании (в {selectedCity.name})</p>
                  ...
                  <li key={ev.id}>
                    <label className="flex cursor-pointer items-start gap-2">
                      <input
                        ...
                        onChange={() => setSelectedEventIds((prev) => toggleId(prev, ev.id))}
                      />
                      <span>
      ...
      <Card id="admin-catalog-cards" className="scroll-mt-20">
        <CardHeader>
          <CardTitle>Карточки каталога · {selectedCity.name}</CardTitle>
          <CardDescription>
            {itemsInCity.length
        ...
        <CardContent>
          {catalogLoading ? (
            ...
            <p className="text-sm text-muted-foreground">
              Нет карточек для «{selectedCity.name}». Создайте выше или переключите город.
            </p>
          ) : (
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {item.status !== "PUBLISHED" ? (
                        <Button
                          ...
                          onClick={() => statusAction.mutate({ id: item.id, action: "publish" })}
                        >
                          Опубликовать
                        </Button>
                      ) : (
                        <Button
                          ...
                          variant="outline"
                          disabled={statusAction.isPending}
                          onClick={() => statusAction.mutate({ id: item.id, action: "unpublish" })}
                        >
                          Снять с публикации
                        </Button>
                      )}
                      {item.status !== "ARCHIVED" ? (
                        <Button
                          ...
                          disabled={statusAction.isPending}
                          onClick={() => {
                            if (confirm("Отправить карточку в архив? Она исчезнет из каталога инструкторов.")) {
                      ...
                      <div className="mt-3 space-y-2 rounded-md border border-dashed border-border p-2">
                            ...
                            <li key={ev.id}>
                              <label className="flex cursor-pointer items-start gap-2">
                                <input
                                  ...
                                  onChange={() => setAttachIds((prev) => toggleId(prev, ev.id))}
                                />
                                <span>
                                  {ev.title}
                                  <span className="block text-xs text-muted-foreground">
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={() =>
                              attachEvents.mutate({ catalogId: item.id, eventIds: attachIds })
                            }
                          >
                            Привязать
                          </Button>
      ...
      <Card id="admin-catalog-published" className="scroll-mt-20">
        <CardHeader>
          <CardTitle>Опубликованные мероприятия · {selectedCity.name}</CardTitle>
          <CardDescription>
