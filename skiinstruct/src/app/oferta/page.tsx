  ...
  return (
    <LegalDocLayout title="Договор бронирования услуг (публичная оферта) о предоставлении платных услуг">
      ...
      <section className="space-y-3">
        ...
        <p className="text-muted-foreground">
          5.4.1. <strong>Привязка карты.</strong> Клиент может сохранить банковскую карту в ЮKassa, чтобы оплачивать
          последующие заказы и записи на события без повторного ввода реквизитов. Списание по сохранённой карте
          выполняется только когда Клиент оформляет заказ или запись на Сайте (не абонентская плата и не списания по
          расписанию). Отключить привязку можно в личном кабинете («Личные данные» → «Отвязать карту») — Исполнитель
          прекращает использовать сохранённый способ оплаты. Нажимая «Привязать карту» или «Оформить заказ и привязать
          карту», Клиент соглашается на сохранение способа оплаты в ЮKassa на этих условиях.
        </p>
        <p className="text-muted-foreground">
      ...
      <section className="space-y-3">
        ...
        <p className="text-muted-foreground">
          6.5. Ответственность за качество, безопасность и содержание занятия несёт <strong>Инструктор</strong>.
        </p>
        <p className="text-muted-foreground">
      ...
      <section className="space-y-3">
        ...
        <p className="text-muted-foreground">
          8.4. Отмена События Инструктором до начала — полный возврат оплатившим участникам. Неявка Инструктора — полный
          возврат клиенту и штраф <strong>{INSTRUCTOR_NO_SHOW_PENALTY_PERCENT}%</strong> с Инструктора.
        </p>
        <p className="text-muted-foreground">
          8.5. <strong>Форс-мажорная отмена События</strong> (после начала времени События): если Событие не состоялось
          по обстоятельствам непреодолимой силы или иным объективным причинам (в том числе неблагоприятные погодные
          ...
          <strong>100%</strong> стоимости участия; штраф с Инструктора не начисляется. Причина сохраняется в записи
          участника и направляется Исполнителю (администратору) для контроля.
        </p>
        <p className="text-muted-foreground">
          8.6. Комиссия Агента по событиям — <strong>{PLATFORM_FEE_PERCENT}%</strong> от стоимости участия каждого
      ...
      <section id="referral" className="space-y-3">
        ...
        <p className="text-muted-foreground">
          9.4. Участник вправе делиться персональной ссылкой в личных сообщениях, в Telegram, VK и на иных площадках,
          где реклама услуг в Российской Федерации не запрещена. <strong>Запрещено</strong> размещать рекламу Сайта,
          реферальной программы и призывы зарегистрироваться (в том числе рилсы, сторис, посты со ссылкой) в Instagram,
          Facebook, Threads и на иных ресурсах организаций, признанных в РФ экстремистскими, а также на площадках, где
          распространение рекламы запрещено законом. Исполнитель вправе не начислять и не выплачивать вознаграждение за
          приглашения, полученные в результате такого размещения, и отказать в участии в программе.
        </p>
      </section>
      ...
      <section className="space-y-3">
        ...
        <p className="text-muted-foreground">
          10.1. Исполнитель не несёт ответственность за вред жизни, здоровью или имуществу Заказчика/Гостя вследствие
        ...
        <p className="text-muted-foreground">
          10.2. В отношении Событий форс-мажор (включая неблагоприятные погодные условия, при которых проведение
          События невозможно или небезопасно) оформляется Инструктором после наступления времени начала События через
          функцию «Форс-мажор отмена» с указанием причины. Последствия для оплаты — по п. 8.5 настоящей Оферты и{" "}
          <Link href={LEGAL_ROUTES.returns} className="text-accent underline">
            Правилам возврата
          </Link>
          .
