...
export default function ReturnsPolicyPage() {
  return (
    <LegalDocLayout title="Правила возврата денежных средств">
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold">3. События</h2>
        <p className="text-muted-foreground">
          3.1. На записи на события (групповые выезды, мастер-классы и т.п.) действуют отдельные правила.
        </p>
        <p className="text-muted-foreground">
          3.2. Кратко: для платных событий оплата производится при записи (предоплата через ЮKassa); запись можно
          отменить до начала. При отмене за{" "}
          <strong>{EVENT_CANCEL_FULL_REFUND_HOURS} ч и более</strong> до начала — полный возврат (если оплата уже
          проведена); менее чем за <strong>{EVENT_CANCEL_FULL_REFUND_HOURS} ч</strong> — место освобождается, оплата не
          возвращается.
        </p>
        <p className="text-muted-foreground">
      ...
      <section className="space-y-3">
        ...
        <p className="text-muted-foreground">
          5.3. По событиям комиссия платформы ({PLATFORM_FEE_PERCENT}% от стоимости участия каждого клиента)
          удерживается при расчётах после проведения события; при полном возврате участнику возвращается уплаченная
          им сумма целиком.
        </p>
      </section>
