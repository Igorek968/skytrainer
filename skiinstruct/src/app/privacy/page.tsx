  ...
  return (
    <LegalDocLayout title="Политика обработки персональных данных">
      ...
      <section className="space-y-3">
        ...
        <p className="text-muted-foreground">
          7.1. Оператор передаёт данные <strong>Инструктору</strong> (имя, телефон, адрес/координаты встречи) для
          проведения занятия или мероприятия. По заказу — после принятия заявки и оплаты (либо бесплатной записи). По
          записи на мероприятие — сразу после активной заявки (в том числе до оплаты, если оплата после события).{" "}
          <strong>Клиенту</strong> может быть показан телефон Инструктора, а Инструктору — телефон Клиента, только по
          явному запросу в интерфейсе («Позвонить»), а также доступен чат по заказу/записи. Номер не публикуется в
          открытой ленте мероприятий.
        </p>
        <p className="text-muted-foreground">
