...
export type CalendarCellKind = "outside" | "available" | "buffer" | "lesson";
...
/** Визуальное состояние ячейки: шаблон доступности + занятость из заказов. */
export function resolveCalendarCellVisual(input: {
  ...
  if (!orderIds.length) {
    return { kind: inTemplate ? "available" : "outside", orderIds: [] };
  }
  ...
  return { kind: isLesson ? "lesson" : "buffer", orderIds };
}
