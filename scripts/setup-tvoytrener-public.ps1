...
if ($RegRuUser -and $RegRuPassword) {
  ...
  Write-Host "or use reg.ru panel: DNS -> delete A 93.77.189.9 -> add CNAME."
} else {
  Write-Host ""
  ...
  Write-Host "  2. Удалите A-запись 93.77.189.9 (сейчас из-за неё ERR_CONNECTION_REFUSED)"
  Write-Host "  3. Добавьте CNAME @ -> <tunnel-id>.cfargotunnel.com (из Cloudflare tunnel)"
  Write-Host "  4. CNAME www -> <tunnel-id>.cfargotunnel.com"
  Write-Host ""
